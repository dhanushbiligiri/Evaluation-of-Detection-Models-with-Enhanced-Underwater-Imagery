...
def enhance_images(input_dir, output_dir):
    ...
    for label_file in os.listdir(labels_dir):
        ...
        if not os.path.exists(dest_label_path):  # Check if label file already exists
            shutil.copy2(src_label_path, dest_label_path)  # Proper indentation here

    # Process images
    for image_file in os.listdir(images_dir):
        output_image_path = os.path.join(output_images_dir, image_file)
        if os.path.exists(output_image_path):
            print(f"Skipping existing file {image_file}")
            continue  # Skip processing if output file already exists
        ...
        with torch.no_grad():
            output = model(img_tensor, la_input)  # Use the model to enhance the image
            enhanced_tensor = output[0] if isinstance(output, tuple) else output

        enhanced_img = ToPILImage()(enhanced_tensor.squeeze(0))
        enhanced_img.save(output_image_path)  # Save the enhanced image

        print(f"Processed and saved {image_file}")

# Assuming other setup code and function calls remain the same

train_input_dir = r'/home/campus30/dnarsipu/Downloads/Ours800_yolo_newSplit/train'
train_output_dir = r'/home/campus30/dnarsipu/Downloads/Ours800_yolo_newSplit/Enhanced Train'
